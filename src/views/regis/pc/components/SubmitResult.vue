<template>
  <div>
    <el-checkbox
      ...
      >{{ $t("Regis.onlyShowError") }}</el-checkbox
    >
    <el-table
      ref="result"
      ...
      >
        <template slot-scope="scope">
          <span :class="scope.row.status">
            {{ $t("Regis.status." + scope.row.status) }}
          </span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('Regis.error.label')" align="left">
        <template slot-scope="scope">
          <span v-show="scope.row.error">
            {{ $t("Regis.error." + scope.row.reason || "unknown") }}
          </span>
        </template>
      </el-table-column>
    </el-table>
...
.success {
  border-left: 15px solid #19c096;
  padding-left: 10px;
  color: #19c096;
}
.error {
  border-left: 15px solid #f26d7d;
  padding-left: 10px;
  color: #f26d7d;
  font-weight: bold;
}
